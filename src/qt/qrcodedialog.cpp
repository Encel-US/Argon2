...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("Argon2")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Argon2://%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non Argon2 unit input in lnReqAmount, we generate the URI with Argon2 as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::AR2, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
