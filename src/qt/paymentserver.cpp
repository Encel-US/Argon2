...
const QString BITCOIN_IPC_PREFIX("Argon2:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click Argon2: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start Argon2: click-to-pay handler");
    else
...
{
    // clicking on Argon2: URLs creates FileOpen events on the Mac:
